...
int main()
{
    ...
    // Random generator gen and gap limits between 200 and 500
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<int> gapPosition(200, 500);
    ...
    while (window.isOpen() == true)
    {
        ...
        if(gameover == false)
        {
            ...
            {
                float newX = std::max(tpipe2.getPosition().x, 1600.0f) + 600;
                int randomGapY = gapPosition(gen);
                tpipe1.setPosition(newX, randomGapY - 75);
                bpipe1.setPosition(newX, randomGapY + 75);
            }
            if(tpipe2.getPosition().x <= -tpipe2.getGlobalBounds().width)
            {
                float newX = std::max(tpipe1.getPosition().x, 1600.0f) + 600;
                int randomGapY = gapPosition(gen);
                tpipe2.setPosition(newX, randomGapY - 75);
                bpipe2.setPosition(newX, randomGapY + 75);
            }*/
           if(tpipe1.getPosition().x <= -tpipe1.getGlobalBounds().width)
{
    float newX = std::max(tpipe2.getPosition().x, 1600.0f) + 600;
    ...
    // Top pipe: position so it extends above screen
    tpipe1.setPosition(newX, randomGapY - 75 - tpipe1.getGlobalBounds().height);
    
    // Bottom pipe: position so gap starts at randomGapY + 75
    bpipe1.setPosition(newX, randomGapY + 75);
}
if(tpipe2.getPosition().x <= -tpipe2.getGlobalBounds().width)
{
    ...
    // Top pipe: position so it extends above screen
    tpipe2.setPosition(newX, randomGapY - 75 - tpipe2.getGlobalBounds().height);
